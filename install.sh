...
config_after_install() {
    ...
    if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
        ...
        echo -e "${red}Canceled, all settings are default settings, please modify in time${plain}"
    fi
}
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Failed to download x-ui, please make sure your server is able to download GitHub files${plain}"
            exit 1
        fi
    else
        last_version=$1
    ...
    config_after_install
    #echo -e "If it is a fresh installation, the default web port is ${green}54321${plain}，Username and password are both by default ${green}admin${plain}"
    #echo -e "Please make sure this port is not used by other programs，${yellow}And make sure port 54321 is allowed${plain}"
    ...
    #echo -e ""
    #echo -e "In the case of the update panel, access the panel as you did before"
    #echo -e ""
