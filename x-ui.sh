...
update() {
    confirm "This function will force the latest version to be reinstalled without data loss. Do you want to continue?" "n"
    if [[ $? != 0 ]]; then
        LOGE "已取消"
        if [[ $# == 0 ]]; then
            before_show_menu
...
ssl_cert_issue() {
    ...
    LOGI "This script will use the Acme script to apply for a certificate, and you need to ensure that it is used:"
    ...
    confirm "I have confirmed above[y/n]" "y"
    if [ $? -eq 0 ]; then
        cd ~
